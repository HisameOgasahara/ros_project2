...
현재 `work_ref_jetson_ready`에서는 두 가지 방식으로 쓸 수 있습니다.

- 로컬/핫스팟 확인: Jetson이 `phone_ws_client.html`을 직접 제공하고 폰이 `ws://JETSON_IP:3000`에 붙습니다.
- GitHub Pages/Cloudflare 확인: GitHub Pages의 `debug.html`이 `wss://...trycloudflare.com`으로 echo 서버에 붙습니다.

실제 로봇 명령 브릿지와 충돌하지 않도록 Cloudflare echo 테스트는 기본 WebSocket 포트 `3001`, HTTP 포트 `8001`을 사용합니다.
...
실제 delivery bridge를 열기 전에 외부 WebSocket 경로만 확인할 때 사용합니다.
...
스크립트는 내부적으로 아래처럼 echo 서버를 띄웁니다.
...
그리고 `cloudflared tunnel --url http://localhost:3001`을 실행합니다.

출력 중 아래 형식의 주소를 복사합니다.

```text
wss://example-random-name.trycloudflare.com
```

GitHub Pages의 `debug.html`을 열고 WebSocket URL에 위 주소를 입력합니다.

```text
https://HisameOgasahara.github.io/ros_webclient/debug.html
```

`Connect` 후 `Send`를 눌렀을 때 `received: echo: hello from github pages`가 나오면 아래 경로는 정상입니다.
...
이 테스트는 ROS2 topic을 건드리지 않습니다. 네트워크/WebSocket 계층만 확인합니다.

포트를 바꾸고 싶으면 실행할 때 환경변수를 줍니다.
