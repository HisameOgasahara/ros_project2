...
HOST = "0.0.0.0"
WS_PORT = int(os.environ.get("WS_PORT", "3000"))
...
def ensure_html_file():
    ...
    HTML_FILE.write_text(HTML_TEMPLATE.replace(":3000", f":{WS_PORT}"), encoding="utf-8")
...
def print_startup_diagnostics():
    ...
    print(f"phone should open: http://<JETSON_HOTSPOT_IP>:{HTTP_PORT}/phone_ws_client.html")
    print(f"phone WebSocket target should be: ws://<JETSON_HOTSPOT_IP>:{WS_PORT}")
    ...
    print("  hostname -I")
    print(f"  ss -ltnp | grep -E '{WS_PORT}|{HTTP_PORT}'")
    print(f"Expected listeners: 0.0.0.0:{WS_PORT} and 0.0.0.0:{HTTP_PORT}")
    print("")
...
def check_port_available():
    for port in (WS_PORT, HTTP_PORT):
        ...
        try:
            ...
            print(f"Check with: ss -ltnp | grep -E '{WS_PORT}|{HTTP_PORT}'")
        finally:
            sock.close()
